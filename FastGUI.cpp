...
void Fast::Window(const char * name, FastVec2 pos)
{
	if (FastInternal::Inited())
	{
		...
		//	win->Draw();
		//}
		FastInternal::FastWindow win(name);
		win.rect.x = pos.x;
		win.rect.y = pos.y;
		win.rect.w = 150;
		win.rect.h = 200;
...
{
	creation = new FastCreation(); creation->Start();
	io = new FastIO();			   io->Start();
	style = new FastStyle();	   style->Start();
	fonts = new FastFonts();	   fonts->Start();
	draw = new FastDraw();		   draw->Start(); 
}

void FastInternal::FastMain::CleanUp()
{
	creation->CleanUp(); FAST_DEL(creation);
	io->CleanUp();		 FAST_DEL(io);
	style->CleanUp();    FAST_DEL(style);
	fonts->CleanUp();    FAST_DEL(fonts);
	draw->CleanUp();     FAST_DEL(draw);
}
...
void FastInternal::FastCreation::Start()
{
}

void FastInternal::FastCreation::CleanUp()
{
}
...
void FastInternal::FastWindow::Draw()
{
	...
	fast_main->draw->Quad(rect.Pos(), FastVec2(rect.Size().x, 20), FastColour(0.4, 0.4, 0.4, 1));
